...
let selectiveMap = function(arr, one, two) {
    //this array will become the array containing the desired elements
    let res = [];

    /* check each element, "el," to see if one(el) is false. if one(el) is false, push this element to the res array.
    otherwise, perform "two" on the element and then push that result, "two(el)," to the res arr */
    arr.forEach((el) => {
       !one(el) ? res.push(el) : res.push(two(el));
    });
    //return the completed res arr.
    return res;
};
